 ...
 */   
      ...
      if (!file_exists($targetDIR . urlencode($oneurl->url) . '.json') && !file_exists($targetDIR . 'errors/' .urlencode($oneurl->url) . '.json')) {
        $success++;
        ...
        try {
          ...
          writeToFile($targetDIR . urlencode($oneurl->url) . '.json', json_encode($masterObj));
         

        } catch (Exception $exception) {
          $errz++;
          // TODO: log this / write as an error file, separately
          writeToFile($targetDIR . 'errors/' . urlencode($oneurl->url) . '.json', json_encode(['error'=>'true']));
